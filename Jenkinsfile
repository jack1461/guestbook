...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="happyit153/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/jack1461/guestbook.git'
            }
        }
        stage('Build') {
            steps {
        ...
        stage('Docker Image Push') {
            steps {
                script {
                    docker.withRegistry('', 'DockerHub_happyit153') {
                        oDockImage.push()
                    }
                }
            }
